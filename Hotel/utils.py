...
def save_to_booking_details_database(data,user):
    ...
    booking_hotel = HotelBookingHistory.objects.create( user=user,
                                        email = data['order']['deliveryInfo'].get('emails', [])[0],
                                        mobile = data['order']['deliveryInfo'].get('contacts', [])[0],
                                        hotel_name = data['itemInfos']['HOTEL']['hInfo'].get('name', ''),
                                        hotel_rating = data['itemInfos']['HOTEL']['hInfo'].get('rt', ''),
                                        longitude = data['itemInfos']['HOTEL']['hInfo']['gl'].get('ln', 0),
                                        latitude =data['itemInfos']['HOTEL']['hInfo']['gl'].get('lt', 0),
                                        address_details = data['itemInfos']['HOTEL']['hInfo']['ad'].get('adr', ''),
                                        city_name = data['itemInfos']['HOTEL']['hInfo']['ad']['city'].get('name', ''),  
                                        state_name = data['itemInfos']['HOTEL']['hInfo']['ad']['state'].get('name', ''),
                                        bookingId =  data['order'].get('bookingId', ''),
                                        osh_bookingId = new_booking_id,
                                        booking_status =  data['order'].get('status', ''),
                                        country_name = data['itemInfos']['HOTEL']['hInfo']['ad']['country'].get('name', ''),
                                        pin_code = data['itemInfos']['HOTEL']['hInfo']['ad'].get('postalCode', 0),
                                        room_id = data['itemInfos']['HOTEL']['hInfo']['ops'][0]['ris'][0].get('id', ''),
                                        room_category = data['itemInfos']['HOTEL']['hInfo']['ops'][0]['ris'][0].get('rc', ''),
                                        room_type = data['itemInfos']['HOTEL']['hInfo']['ops'][0]['ris'][0].get('rt', ''),
                                        adults_no = no_of_adults,
                                        child_no = no_of_child,
                                        meal_base = data['itemInfos']['HOTEL']['hInfo']['ops'][0]['ris'][0].get('mb', ''),
                                        total_price = data['order']['amount'],
                                        taxes_fees = data['itemInfos']['HOTEL']['hInfo']['ops'][0]['ris'][0]['tfcs'].get('TAF', ''),
                                        base_price =data['itemInfos']['HOTEL']['hInfo']['ops'][0]['ris'][0]['tfcs'].get('BF', ''),
